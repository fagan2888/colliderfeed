...
prev_prices = None
def minutely_feed_task(symbols, names, write_keys, budgets ):
    """ Modify feed of price changes or at least keep it warm """
    global prev_prices
    with Timer() as t_get:
        prices = collider_prices(symbols=symbols)
    print( str(t_get.elapsed)+' ms getting data.')

    if prev_prices is None or prices is None:
        set_or_touch(touch=True, names=names, write_keys=write_keys, budgets=budgets)
    else:
        price_changes = [ p-pp for p,pp in zip(prices,prev_prices) ]
        with Timer() as t_put:
            set_or_touch(touch=False, names=names, write_keys=write_keys, values=price_changes, budgets=budgets)
        print(str(t_put.elapsed)+ ' ms putting data.')
